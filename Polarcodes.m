function[EbNodB, FER_sim, BER_sim, Nblkerrs, Nbiterrs, Nblocks] = Polar(X, Y, E)
...
F = Q1(1:N-K); %Frozen Position: Q1(:N-K)
...
for blk = 1:Nblocks
    ...
    f = @(a,b) (1-2*(a<0)).*(1-2*(b<0)).*min(abs(a),abs(b));%f minsum
    ...
    while (done == 0) %traverse till bits are decoded
        %leaf or not
        if depth == n
            if any(F == (node+1)); %node frozen
                ucap(n+1,node+1) = 0;
            else
                if L(n+1,node+1)>=0;
                    ucap(n+1,node+1)=0;
            ...
            if ns(npos) == 0 %step L and go to left child
                ...
                a = Ln(1:temp/2); b = Ln(temp/2+1:end); %split belief into 2
                node =node*2; depth = depth+1; %next node:left child
                temp = temp/2; %incoming belief length for left child
                L(depth+1, temp*node+1:temp*(node+1)) = f(a,b); %minsum & storage
                ns(npos) = 1;
            else
                if ns(npos) == 1 %step R and go to right child
                    ...
                    L(depth+1, temp*node+1:temp*(node+1)) = g(a,b,ucapn); %g & storage
                    ns(npos) = 2;
                else %step u and go to parent
                    temp = 2^(n-depth);
                    lnode = 2*node; rnode = 2*node + 1; cdepth = depth + 1; %left child
                    ctemp = temp/2;
